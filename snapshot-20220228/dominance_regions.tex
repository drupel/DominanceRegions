...
\begin{document}
  ...
  Cluster algebras can be understood geometrically by gluing a collection of algebraic tori along birational morphisms known as mutations.
  While such a description seems simple at first glance, this structure has been found across much of the range of mathematical objects under current consideration in the literature.
  To give an idea of how fundamental the process of cluster mutation has come to be understood within mathematics, we note the following (far from exhaustive list of) areas to which this notion is connected: representation theory of artin algebras, especially reflection functors and tilting theory \cite{???}; representation theory of algebraic groups, especially Coxeter groups, double Bruhat cells, and (dual) canonical bases \cite{???}; hyperbolic geometry of Riemann surfaces \cite{???}; mathematical physics, especially quiver gauge theories \cite{???}; knot theory, especially the Alexander polynomial \cite{some papers of Schiffler}; algebraic geometry, especially log Calabi-Yau varieties \cite{GHK, GHKK}.
  To say the least, any advance in the fundamental understanding of cluster algebras is likely to find broad application throughout mathematics.

  The investigations of this paper are inspired by a study of Qin on the deformability of bases which are well-behaved under mutations.
  More precisely, we study the notion of dominance order on the tropical labellings for bases of cluster algebras pioneered by Qin.
  As no explicit calculation is carried out in his works, we aim here to initiate a thorough study of the dominance regions for cluster algebras.
  We begin in the setting of cluster algebras of rank 2, i.e. containing a pair of mutable cluster variables, with no frozen variables.

  Fix integers $b,c>0$ and assume $bc\ge4$.
  When $bc\le3$, the cluster algebra will be of finite-type and the resulting basis of cluster monomials is not deformable \cite{???}.
  The cluster algebra $\cA(b,c)$ is the $\ZZ$-subalgebra of $\QQ(x_1,x_2)$ generated by \emph{cluster variables} $x_m$, $m\in\ZZ$, defined recursively by
  \begin{equation}
  ...
  That is, each basis element $x_\lambda$, with \emph{$\bfg$-vector} $\lambda\in\ZZ^2$, can be written in the form $\sum\limits_{a_1,a_2 \ge 0} c^\lambda_{a_1,a_2} x_1^{\lambda_1-ba_1} x_2^{\lambda_2+ca_2}$ with $c_{0,0}=1$ and this structure reproduces after expanding in terms of any pair $\{x_k,x_{k+1}\}$.

  More specifically, we compute the possible transitions between any two pointed bases.
