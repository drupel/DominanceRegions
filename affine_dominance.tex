...
\section{Iterated Mutations}
  ...
  Below we freely identify each vertex $(\ell,0)$ with $\ell$.

  Given a path $\fp\subset Q^{(r)}_\beps$, write $\wt(\fp)$ for the product of the weights along the arrows in $\fp$.
  Let $i(\fp)$ and $t(\fp)$ denote the initial and terminal vertex along the path $\fp$.
  Below we only consider paths $\fp\subset Q^{(r)}_\beps$ with $i(\fp)=(\ell,0)$ for some $\ell$.
  \begin{proposition}
    Consider $r>0$ with $r=ns+\ell$ for $1\le\ell\le n$.
    For $\lambda\in\ZZ^n$, suppose the tropical transformation $\phi_{t_r}(\tilde\lambda)=(\lambda'_1,\ldots,\lambda'_n,\lambda'_{n+1},\cdots,\lambda'_{2n})$ is given by
    \[
      E^{t_{r-1}}_{\ell,\varepsilon_r} \cdots E^{t_{r-\ell}}_{1,\varepsilon_{r-\ell+1}} E^{t_{ns-1}}_{n,\varepsilon_{ns}} \cdots E^{t_1}_{2,\varepsilon_2} E^{t_0}_{1,\varepsilon_1} \lambda
    \]
    ...
    \]
    and $\lambda'_{n+k}=0$ for $1\le k\le n$ and $r<0$ if $\varepsilon_i=-$ for all $i$.
    \sayDR{We need this for arbitrary sequences of signs and to include $r>0$.}
  \end{proposition}
  \begin{proof}
    We work by induction on $r$.
    Write $r=ns+\ell$.
  ...
  \[\lambda'_k=\begin{cases}\sum\limits_{\substack{p\subset\widehat{Q}_\beps\\ \hgt(p)=-s-1, t(p)=(k,0)}}\wt(p)\lambda_{i(p)} & \text{if $1\le k\le\ell-1$;}\\ \sum\limits_{\substack{p\subset\widehat{Q}\\ \hgt(p)=-s, t(p)=(k,0)}}\wt(p)\lambda_{i(p)} & \text{if $\ell\le k\le n$;} \end{cases}\]
