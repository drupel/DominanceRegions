...
\section{Iterated Mutations}
  ...
  \begin{lemma}
    For $r\in\ZZ_{\ge0}$ with $r=ns+\ell$ for $1\le \ell \le n$, the matrix $\tilde B^{t_r}$ has principal part given by $b^{t_r}_{ij}=(-1)^{\epsilon(i,\ell)+\epsilon(j,\ell)}b_{ij}$ for $\epsilon(i,j)=\begin{cases} 1 & \text{if $i\le j$;}\\ 0 & \text{otherwise;}\end{cases}$ and the $\bfc$-vector part of the $k$-th column is given by $\bfc^{(k)}_r$ for $1\le k\le n$.
  \end{lemma}
  \begin{proof}
    ...
    In addition, we see that the $\ell$-th $\bfc$-vector is given by $-\bfc^{(\ell)}_{r-1}=\bfc^{(\ell)}_r$ as desired.
    ...
    It then follows from \eqref{eq:matrix mutation} that the $k$-th $\bfc$-vector of $\tilde B^{t_r}$ is given by $\bfc^{(k)}_{r-1}+b_{\ell k}\bfc^{(\ell)}_{r-1}$ if $\ell<k$ and by $\bfc^{(k)}_{r-1}-b_{\ell k}\bfc^{(\ell)}_{r-1}$ if $\ell>k$.
    Each of these is equal to $\bfc^{(k)}_r$ as desired.
  \end{proof}
  ...
  \begin{itemize}
    \item if $\varepsilon=+$, we have $\bfc^{(k)}_r$ if $r\not\equiv_n k$ and zero otherwise;
    \item if $\varepsilon=-$, we have $-\bfc^{(k)}_r$ if $r\equiv_n k$ and zero otherwise.
  \end{itemize}
