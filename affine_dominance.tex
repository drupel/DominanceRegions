...
\section{Iterated Mutations}
  ...
  Define vectors $\bfc^{(k)}_r\in\ZZ^n$ for $1\le k\le n$ and $r\in\ZZ_{\ge0}$ recursively as follows:
  \sayDR{A slight modification is needed to handle the opposite mutation sequence.}
  \begin{itemize}
    \item $\bfc^{(k)}_0=\bfe_k$ is the standard basis vector 
    \item for $1\le \ell \le n$, we have $\bfc^{(k)}_{ns+\ell}=\begin{cases} -\bfc^{(k)}_{ns+\ell-1} & \text{if $\ell=k$;}\\ \bfc^{(k)}_{ns+\ell-1}+b_{\ell k}\bfc^{(\ell)}_{ns+\ell-1} & \text{if $\ell<k$;}\\ \bfc^{(k)}_{ns+\ell-1}-b_{\ell k}\bfc^{(\ell)}_{ns+\ell-1} & \text{if $\ell>k$.} \end{cases}$
  \end{itemize}
  ...
  \begin{lemma}
    ...
    For $r\in\ZZ_{\ge0}$, the vector $\bfc^{(k)}_r$ is non-negative if $r\not\equiv_n k$ and non-positive when $r\equiv_n k$.
  \end{lemma}
  \begin{proof}
    Induction?
  \end{proof}
  ...
  \begin{lemma}
    For any $s\in\ZZ_{\ge0}$ and $0\le \ell \le n-1$, the matrix $\mu_\ell\cdots\mu_1(\mu_n\cdots\mu_1)^s\tilde B$ has principal part given by $(b'_{ij})$ where $b'_{ij}=(-1)^{\epsilon(i,\ell)+\epsilon(j,\ell)}b_{ij}$ for $\epsilon(i,j)=\begin{cases} 1 & \text{if $i\le j$;}\\ 0 & \text{otherwise;}\end{cases}$ and the $\bfc$-vector part of the $k$-th column is given by $\bfc^{(k)}_{ns+\ell}$ for $1\le k\le n$.
  \end{lemma}
  \begin{proof}
    We work by induction on $ns+\ell$.
    For $s=\ell=0$, we just get $\tilde B$ and the claim clearly holds.
    Assume the claim holds for a chosen $s$ and $\ell-1$, we show the claim holds for $s$ and $\ell$ (the case $\ell=n$ gives rise to the claim for $s+1$ and $\ell=0$).
    Write $\tilde B'=\mu_{\ell-1}\cdots\mu_1(\mu_n\cdots\mu_1)^s\tilde B$ and let $Q'_B$ denote the corresponding acyclic quiver.

    By \eqref{eq:matrix mutation}, the mutation $\mu_\ell$ reverses the sign of the $\ell$-th row and column of $\tilde B'$.
    It follows that the principal part of $\mu_\ell\tilde B'$ is given as follows:
    \begin{itemize}
      \item if $i,j<\ell$, or $i<\ell$ and $j>\ell$, or $i<\ell$ and $j>\ell$, then $(-1)^{\epsilon(i,\ell)+\epsilon(j,\ell)}b_{ij}=(-1)^{\epsilon(i,\ell-1)+\epsilon(j,\ell-1)}b_{ij}$;
    ...
    In addition, we see that the $\ell$-th $\bfc$-vector is given by $-\bfc^{(\ell)}_{ns+\ell-1}=\bfc^{(\ell)}_{ns+\ell}$ as desired.
    By Lemma~\ref{lem:tropical signs}, the $\ell$-th $\bfc$-vector of $\tilde B'$ is non-negative.  
    Observe further that vertex $\ell$ is a sink in $Q'_B$ so that the $\ell$-th row of $\tilde B'$ has non-negative entries, in particular we see that $b'_{ij}=-b_{ij}$ for $j<\ell$ and $b'_{ij}=b_{ij}$ for $j>\ell$.
    It then follows from \eqref{eq:matrix mutation} that the $k$-th $\bfc$-vector of $\mu_\ell\tilde B'$ is given by $\bfc^{(k)}_{ns+\ell-1}+b_{\ell k}\bfc^{(\ell)}_{ns+\ell-1}$ if $\ell<k$ and by $\bfc^{(k)}_{ns+\ell-1}-b_{\ell k}\bfc^{(\ell)}_{ns+\ell-1}$ if $\ell>k$.
    Each of these is equal to $\bfc^{(k)}_{ns+\ell}$ as desired.
  \end{proof}
