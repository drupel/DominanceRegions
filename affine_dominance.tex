...
\section{Iterated Mutations}
  ...
  Define vectors $\bfc^{(r)}_k\in\ZZ^n$ for $1\le k\le n$ and $r\in\ZZ_{\ge0}$ recursively as follows:
  \sayDR{A slight modification is needed to handle the opposite mutation sequence.}
  \begin{itemize}
    \item $\bfc^{(0)}_k=\bfe_k$ is the standard basis vector 
    \item for $r=ns+\ell$ with $1 \le \ell \le n$, we have $\bfc^{(r)}_k=\begin{cases} -\bfc^{(r-1)}_k & \text{if $\ell=k$;}\\ \bfc^{(r-1)}_k+b_{\ell k}\bfc^{(r-1)}_\ell & \text{if $\ell<k$;}\\ \bfc^{(r-1)}_k-b_{\ell k}\bfc^{(r-1)}_\ell & \text{if $\ell>k$.} \end{cases}$
  \end{itemize}
  ...
  \begin{lemma}
    ...
    For $r\in\ZZ_{\ge0}$, the vector $\bfc^{(r)}_k$ is non-negative if $r\not\equiv_n k$ and non-positive when $r\equiv_n k$.
  \end{lemma}
  \begin{proof}
    Induction?
  \end{proof}
  ...
  \begin{lemma}
    ...
    The limiting ray $\lim_{s\to\infty} \RR_{\ge0}\bfc^{(r)}_k$ is (up to sign) independent of $k$ and $\ell$ and is spanned by an eigenvector for $\tau$ acting on roots corresponding to the largest real eigenvalue (or maybe its inverse).
  \end{lemma}
  \begin{proof}
  ...
  For $\lambda\in\ZZ^n$ and $r\in\ZZ_{\ge0}$ with $r=ns+\ell$ for $1\le\ell\le n$, the tropical transformation $\phi_{t_r}(\lambda)$ is given by
  \[E^{t_{r-1}}_{\ell,\varepsilon_{r-1}} \cdots E^{t_{r-\ell}}_{1,\varepsilon_{r-\ell}} E^{t_{ns-1}}_{n,\varepsilon_{ns-1}} \cdots E^{t_1}_{2,\varepsilon_1} E^{t_0}_{1,\varepsilon_0} \lambda\]
  for a sequence of signs $\varepsilon_0,\ldots,\varepsilon_{r-1}$ depending on $\lambda$.
  \begin{lemma}
    If $\lambda\in\ZZ^n$ is imaginary, then every sign $\varepsilon_j$ above is equal to $-$.
    \sayDR{Can we use this as a definition of imaginary $\bfg$-vectors?}
  \end{lemma}
  \begin{proof}
    With $1,\ldots,n$ being sink adapted, this choice gives the most complicated transformation at each step.
  ...
  \begin{lemma}
    For any imaginary $\bfg$-vector $\lambda\in\ZZ^n$ and any source or sink adapted sequence of mutations, the coefficient part of
    \[E^{t_{r-1}}_{\ell,\varepsilon_{r-1}} \cdots E^{t_{r-\ell}}_{1,\varepsilon_{r-\ell}} E^{t_{ns-1}}_{n,\varepsilon_{ns-1}} \cdots E^{t_1}_{2,\varepsilon_1} E^{t_0}_{1,\varepsilon_0} \tilde\lambda\]
    stabilizes after the first $n$ mutations.
  \end{lemma}
  \begin{proof}
