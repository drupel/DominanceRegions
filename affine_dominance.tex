...
\section{Iterated Mutations}
  ...
  \[\{(\ell,s)\in [n]\times\ZZ_{\ge0}:ns+\ell\le n+r\}\cup\{k:n+1\le k\le 2n\}\]
  ...
  \begin{itemize}
    ...
    \item an arrow $(\ell,s)\to k$ of weight $\varepsilon_{ns+\ell}\bfc^{(ns+\ell)}_{k\ell}$ whenever $\varepsilon_{ns+\ell}=\sgn(\bfc^{(ns+\ell)}_{k\ell})$.
  \end{itemize}
  For $1\le\ell\le r$, write $\hat{\ell}$ for the vertex $(\ell,s)$ of $Q^{(r)}_\beps$ with $s$ maximal.
  For $n+1\le k\le 2n$, $\hat{k}=k$.
  ...
  \[\{(\ell,s)\in [n]\times\ZZ_{<0}:-n-r<ns+\ell\}\cup\{k:n+1\le k\le 2n\}\]
  and having the following arrows:
  \begin{itemize}
  ...
  Below we only consider paths $\fp\subset Q^{(r)}_\beps$ with $i(\fp)=(\ell,0)$ for some $\ell$ and paths $\fp\subset Q^{(-r)}_\beps$ with $i(\fp)=(\ell,-1)$ for some $\ell$ or paths with $i(\fp)=k$ for some $n+1\le k\le 2n$.
  \begin{proposition}
