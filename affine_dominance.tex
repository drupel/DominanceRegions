...
\section{Iterated Mutations}
  ...
  \begin{proposition}
    ...
    \begin{itemize}
      \item For $r>0$ with $r=ns+\ell$, we have (this is for $s=0$ so far)
        \begin{equation}
          \label{eq:positive g-vector mutations}
          \lambda'_k=
            \begin{cases} 
              \lambda_k+\sum\limits_{1\le i_1<\cdots<i_m\le \ell} (-1)^m b_{ki_m} \prod\limits_{p=2}^m b_{i_p i_{p-1}}\lambda_{i_1} & \text{if $\ell+1\le k\le n$;}\\
              -\lambda_k-\sum\limits_{1\le i_1<\cdots<i_m\le \ell} (-1)^m b_{ki_m} \prod\limits_{p=2}^m b_{i_pi_{p-1}}\lambda_{i_1} & \text{if $1\le k\le \ell$;}\\
            \end{cases}
        \end{equation}
        and $\lambda'_{n+k}=0$ for $1\le k\le n$.
      \item For $-n\le r\le -1$, we have $\lambda'_{n+k}=0$ for $1\le k\le n+r$ and \[\lambda'_{n+k}=\lambda_k+\sum\limits_{k<i_1<\cdots<i_m\le n} b_{ki_1} \prod\limits_{p=2}^m b_{i_{p-1}i_p}\lambda_{i_m}\]
      for $n+r+1\le k\le n$.
      \item For any $r<-n$, we have 
        \[\lambda'_{n+k}=\lambda_k+\sum\limits_{k<i_1<\cdots<i_m\le n} b_{ki_1} \prod\limits_{p=2}^m b_{i_{p-1}i_p}\lambda_{i_m}\]
        for $1\le k\le n$.
    \end{itemize}
  ...
  \begin{lemma}
    ...
    For $r\in\ZZ$, write $\phi_{t_r}(\tilde\lambda)=(\lambda'_1,\ldots,\lambda'_{2n})$.
    \begin{itemize}
      \item For any $r\ge0$, we have $\lambda'_{n+s}=0$ for $1\le s\le n$.
      \item For $-n\le r\le -1$, we have $\lambda'_{n+s}=0$ for $1\le s\le n+r$ and \[\lambda'_{n+s}=\lambda_s+\sum\limits_{s<i_1<\cdots<i_m\le n} b_{si_1} \prod\limits_{p=2}^m b_{i_{p-1}i_p}\lambda_{i_m}\]
      for $n+r+1\le s\le n$.
      \item For any $r<-n$, we have 
        \[\lambda'_{n+s}=\lambda_s+\sum\limits_{s<i_1<\cdots<i_m\le n} b_{si_1} \prod\limits_{p=2}^m b_{i_{p-1}i_p}\lambda_{i_m}\]
        for $1\le s\le n$.
    \end{itemize}
  \end{lemma}
  \begin{proof}
    We work by induction on $r\in\ZZ_{\ge0}$.
    For $r=0$, the claim is immediate from the definition of $\tilde\lambda$.
    Following Lemma~\ref{lem:imaginary mutations}, for $r>0$ we have $\varepsilon_r=-$.
    Writing $r=ns+\ell$, from Lemma~\ref{lem:tropical signs} and Lemma~\ref{lem:preprojective exchange matrices} we see that the $\ell$-th column of $\tilde B^{t_{r-1}}$ has $\bfc$-vector part given by $\bfc^{(r-1)}_\ell$ which is a positive vector.
    It follows that the matrix $E^{t_{r-1}}_{\ell,\varepsilon_r}$ acts as the identity in the last $n$ components and the claim for $r$ follows from the claim for $r-1$.

    An analogous argument shows that the coefficient part of $\phi_{t_{-r}}(\tilde\lambda)$ stabilizes for $r\in\ZZ_{>n}$.

  \end{proof}

  For $1\le r\le n$, write $\phi_{t_{-r}}(\tilde\lambda)=(\lambda'_1,\ldots,\lambda'_{2n})$.
  Then we claim that
  \[
    \lambda'_s=
    \begin{cases} 
      \lambda_s+\sum\limits_{n-r<i_1<\cdots<i_m\le n} b_{si_1} \prod\limits_{p=2}^m b_{i_{p-1}i_p}\lambda_{i_m} & \text{if $1\le s\le n-r$;}\\
      -\lambda_s-\sum\limits_{n-r<i_1<\cdots<i_m\le n} b_{si_1} \prod\limits_{p=2}^m b_{i_{p-1}i_p}\lambda_{i_m} & \text{if $n-r+1\le s\le n$;}\\
      \lambda_{s-n}+\sum\limits_{s-n<i_1<\cdots<i_m\le n} b_{s-n,i_1} \prod\limits_{p=2}^m b_{i_{p-1}i_p}\lambda_{i_m} & \text{if $2n-r+1\le s\le 2n$;}\\
      0 & \text{if $n+1\le s\le 2n-r$.}
    \end{cases}
  \]

  For $1\le r\le n$, write $\phi_{t_r}(\lambda)=(\lambda'_1,\ldots,\lambda'_n)$.
  Then we claim that
  \[
    \lambda'_s=
    \begin{cases} 
      \lambda_s+\sum\limits_{1\le i_1<\cdots<i_m\le r} (-1)^m b_{si_m} \prod\limits_{p=2}^m b_{i_p i_{p-1}}\lambda_{i_1} & \text{if $r+1\le s\le n$;}\\
      -\lambda_s-\sum\limits_{1\le i_1<\cdots<i_m\le r} (-1)^m b_{si_m} \prod\limits_{p=2}^m b_{i_pi_{p-1}}\lambda_{i_1} & \text{if $1\le s\le r$;}\\
    \end{cases}
  \]
