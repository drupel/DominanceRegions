...
\label{sec:maximum support} 
  ...
  \begin{definition}
    A subset $\cS\subset\RR^2$ is called \scalebox{1.7}{$\llcorner$}-closed  \emph{min-convex} if $\lambda,\mu\in\cS$ implies the segments joining $\lambda$ and $\mu$ to $\big(\min(\lambda_0,\mu_0),\min(\lambda_1,\mu_1)\big)$ are contained in $\cS$.
    %$\lefthalfcup$
  \end{definition}
