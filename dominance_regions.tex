...
\begin{document}
  ...
  Write $\Im(B)$ for the span of the columns of $B$ inside $\RR^n$ and $\Im^+(B)$ for their non-negative span.
  Define $\Im_\ZZ(B)$ and $\Im^+_\ZZ(B)$ analogously over the integers.
  ...
  In the case when $B$ is acyclic, there exists a labeling $\{k_1,\ldots,k_n\}=\{1,\ldots,n\}$ so that $\ell<\ell'$ implies $b_{k_\ell k_{\ell'}}\ge 0$, such a sequence is called \emph{sink-adapted}.
  ...
  \begin{lemma}
    ...
    If $\lambda\in\Im(B^t)$, then $\phi^t_k(\lambda)\in\Im(B^{t'})$.
  \end{lemma}
  \begin{proof}
    ...
    so that $\phi^t_k(\lambda)\in\Im(B^{t'})$.
  \end{proof}
  ...
  \end{itemize}
  \begin{lemma}
    Suppose $B$ is acyclic with sink-adapted sequence $k_1,\ldots,k_n$.
    ...
    For a sequence of signs $\varepsilon_1,\ldots,\varepsilon_n$, the product $E^{t_{n-1}}_{k_n,\varepsilon_n}\cdots E^{t_0}_{k_1,\varepsilon_1}$ is given by $A=(a_{ij})$ where
    \[a_{k_{\ell'} k_\ell}=\sum_{U=\{\ell=\ell_1<\ldots<\ell_t\}\subseteq[\![\ell,n]}\prod_{s=1}^t \begin{cases} [-\varepsilon_{\ell_t}b_{k_{\ell'} k_{\ell_t}}]_+ & \text{if $s=t$ and $\ell_t>\ell'$}\\ -1 & \text{if $s=t$ and $\ell_t=\ell'$}\\ [\varepsilon_{\ell_s} b_{k_{\ell_{s+1}} k_{\ell_s}}]_+ & \text{if $s<t$ and $\ell_t\ge\ell'$}\\ 0 & \text{if $\ell'>\ell_t$} \end{cases} \]
    
    Then $\phi_{t_n}$ is the following piecewise-linear automorphism:
    \begin{equation}
      \label{eq:tropical twist}
      \text{(This is going to require casework with the possible signs, we'll really only need it for imaginary $\bfg$-vectors.)}
    \end{equation}
  \end{lemma}
  \begin{proof}
    We prove the following modified formula for the product $E^{t_{m-1}}_{k_m,\varepsilon_n}\cdots E^{t_0}_{k_1,\varepsilon_1}$ with $1\le m\le n$ by induction on $m$.
    Writing this matrix as $A^{(m)}=(a^{(m)}_{ij})$, we claim that
    \[a^{(m)}_{k_{\ell'} k_\ell}=\sum_{U=\{\ell=\ell_1<\ldots<\ell_t\}\subseteq[\![\ell,n]}\prod_{s=1}^t \begin{cases} [-\varepsilon_{\ell_t}b_{k_{\ell'} k_{\ell_t}}]_+ & \text{if $s=t$ and $m\ge\ell_t>\ell'$}\\ -1 & \text{if $s=t$ and $m\ge\ell_t=\ell'$}\\ [\varepsilon_{\ell_s} b_{k_{\ell_{s+1}} k_{\ell_s}}]_+ & \text{if $s<t$ and $\ell_t\ge\ell'$}\\ [\varepsilon_\ell b_{k_{\ell'} k_\ell}]_+ & \text{if $\ell'>m\ge\ell_t=\ell$}\\ 1 & \text{if $\ell=\ell_t=\ell'>m$}\\ 0 & \text{if $m\ge\ell'>\ell_t$ or $\ell_t>\ell'>m$} \end{cases} \]
    Observe that the additional conditions vanish when $m=n$, proving the desired result once this formula is established.

    Since the sequence $k_1,\ldots,k_n$ is sink-adapted, the matrix $E^{t_{\ell-1}}_{k_\ell,\varepsilon_\ell}$, for $1\le\ell\le n$, has entries $e^{(\ell)}_{ij}$ given by
    \begin{equation}
      \label{eq:sink mutation matrix}
      e^{(\ell)}_{ij}=\begin{cases} 1 & \text{if $i=j\ne k_\ell$;}\\ -1 & \text{if $i=j=k_\ell$;}\\ [-\varepsilon_\ell b_{ik_\ell}]_+ & \text{if $i=k_{\ell'}$, $j=k_\ell$, $\ell>\ell'$;}\\ [\varepsilon_\ell b_{ik_\ell}]_+ & \text{if $i=k_{\ell'}$, $j=k_\ell$, $\ell'>\ell$;}\\ 0 & \text{otherwise;} \end{cases}
    \end{equation}
    the entries for $\ell'<\ell$ picking up this sign from the prior matrix mutations.
    From this, it is easy to see that $a^{(1)}_{k_{\ell'} k_\ell}=e^{(1)}_{k_{\ell'} k_\ell}$ for $\ell,\ell'\in[1,n]$, this establishes the base of the induction on $m$.

    Now suppose the matrix $A^{(m)}$ is given as above.
    We need to check that 
    \begin{equation}
      \label{eq:twist matrix recursion}
      a^{(m+1)}_{k_{\ell'} k_\ell}=\sum\limits_{\ell''=1}^n e^{(m+1)}_{k_{\ell'} k_{\ell''}} a^{(m)}_{k_{\ell''} k_\ell}.
    \end{equation}
    It will be convenient to distinguish cases:
    \begin{itemize}
      \item For $\ell'=m+1$, the right hand side of \eqref{eq:twist matrix recursion} becomes $-a^{(m)}_{k_{m+1} k_\ell}$.
      \item If $\ell'>m+1$, the right hand side of \eqref{eq:twist matrix recursion} becomes $a^{(m)}_{k_{\ell'} k_\ell}+[\varepsilon_{m+1} b_{k_{\ell'} k_{m+1}}]_+ a^{(m)}_{k_{m+1} k_\ell}$.
      \item If $m+1>\ell'$, the right hand side of \eqref{eq:twist matrix recursion} becomes $a^{(m)}_{k_{\ell'} k_\ell}+[-\varepsilon_{m+1} b_{k_{\ell'} k_{m+1}}]_+ a^{(m)}_{k_{m+1} k_\ell}$.
    \end{itemize}
  \end{proof}
