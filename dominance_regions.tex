...
\begin{document}
  ...
  For $h\in D_1$, define the \emph{height} $\hgt(h)$ as the number of $v\in D_2$ with $v<h$.
  For $v\in D_2$, define the \emph{depth} $\dpt(v)$ as the number of $h\in D_1$ with $h<v$.
  ...
  \begin{definition}
    ...
    Given a Dyck path $D=D[a_1,a_2]$ which is vertically-adapted to $c$, define a Dyck path $\mu_{1,c}(D)$ inside the lattice rectangle $[0,ca_1-a_2]\times[0,a_1]$ by replacing each horizontal edge of $D$, together with the $\ell$ vertical edges which immediately follow it, by $c-\ell$ horizontal edges followed by a vertical edge.
    ...
    Given a Dyck path $D=D[a_1,a_2]$ which is horizontally-adapted to $b$, define a Dyck path $\mu_{2,b}(D)$ inside the lattice rectangle $[0,a_2]\times[0,ba_2-a_1]$ by replacing each vertical edge of $D$, together with the $\ell$ horizontal edges which immediately precede it, by a horizontal edge followed by $b-\ell$ vertical edges.
  ...
  However since we work in arbitrary Dyck paths instead of just the maximal ones, we recall and adapt the proofs as needed.

  For $e \le e'\in D$, write $ee'$ for the subpath of $D$ beginning with $e$ and ending with $e'$.
  %Take $\overline{e}e'$ and $e\overline{e}'$ to be the subpaths of $ee'$ with $e$ and $e'$ removed, respectively.
  When $e=e'$, we take $ee'$ to be the subpath consisting of only the edge $e$.
  For $e \le h\in D$ with $h\in D_1$, write $eh^{\!\uparrow}$ for the subpath $eh$ together with all vertical edges immediately following $h$.
  For $v \le e\in D$ with $v\in D_2$, write ${}^\leftarrow\!\!\! ve$ for the subpath $ve$ together with all horizontal edges immediately preceding $v$.
  \begin{definition}
  ...
  \begin{lemma}
    Suppose the Dyck path $D$ is vertically-adapted to $c$ and let $D'=\mu_{1,c}(D)$ with $D'_2=\{v'_1,\ldots,v'_{a_1}\}$.
    For $H\subset D_1$, define $V'\subset D'_2$ by $V'=D'_2\setminus \mu_{1,c}(H)$.
    Then for any $h_i<h_j \in D_1$, we have $f_H(h_i h_j^{\!\uparrow})=-\vv{f}_{V'}({}^\leftarrow\!\!\! v'_i v'_j)$.
  \end{lemma}
  \begin{proof}
  ...
  \begin{definition}
    ...
    \begin{itemize}
      \item For $h\in D_1$, write $D(h;H)$ (resp. $D_\WA(h;H)$) for the shortest subpath $he \subset D$ (resp. $he \subset D_\WA$) with $f_H(he)=0$ or, if no such subpath exists, set $D(h;H)=hv_{a_2}$ (resp. set $D_\WA(h;H)=D$).
      \item For $v\in D_2$, write $D(v;V)$ (resp. $D_\WA(v;V)$) for the shortest subpath $ev \subset D$ (resp. $ev \subset D_\WA$) with $f_V(ev)=0$ or, if no such subpath exists, set $D(v;V)=h_1 v$ (resp. set $D_\WA(v;V)=D$).
    \end{itemize}
    Write 
    \[D(H):=\bigcup_{h\in H} D(h;H) \qquad \text{and} \qquad D(V):=\bigcup_{v\in V} D(v;V).\]
    Define the \emph{shadow} of $H$ (resp. \emph{shadow} of $V$) as the set $\sh_D(H):=D(H)\cap D_2$ (resp. $\sh_D(V):=D(V)\cap D_1$) and the \emph{local shadow} of $h\in H$ (resp. of $v\in V$) as the set $\sh_D(h;H)=D(h;H)\cap D_2$ (resp. $\sh_D(v;V)=D(v;V)\cap D_1$).
    Define shadows and local shadows for $D_\WA$ similarly.
  \end{definition}
  ...
  \begin{proof}
    We prove (1), the proof of (2) is similar.
    By Lemma~\ref{le:shadow statistics}, $f_H(he)>0$ for every proper subpath of $D(h;H)=hv$ and so the condition \eqref{eq:HGC} is not satisfied for the path $hv$, but then, since $V\in\cC_D(H)$, the condition \eqref{eq:VGC} must be satisfied for this path.
  \end{proof}
  ...
  \begin{proof}
    We prove (1), the proof of (2) is similar.
    For $h=h'$ or $D(h;H)=D$ or $D(h';H)=D$ there is nothing to show, so assume both shadow paths are proper subpaths of $D$.
    Suppose the intersection of $D(h;H)$ and $D(h';H)$ is nonempty, say with $h'\in D(h;H)=he$.
    If $e=v_{a_2}$, then $D(h';H)=h'v_{a_2}\subset D(h;H)$.
    Otherwise, denote by $e'\in D$ the edge immediately preceding $h'$ in $he$.
    If $e\in D(h';H)$, Lemma~\ref{le:shadow statistics} shows that $f_H(he)=f_H(he')+f_H(h'e)>0$, in contradiction to the definition of $D(h;H)$.
  \end{proof}

  \begin{definition}
    Consider $H\sqcup V\subset D$ with $H\subset D_1$ and $V\subset D_2$.
    \begin{enumerate}
      \item Define $\rsh_D(H)\subset\sh_D(H)$, the \emph{remote shadow} of $H$, by removing for each $d\in[1,a_1]$ the (up to) $c$ vertical edges of depth $d$ immediately following $h_d$.
      \item Define $\rsh_D(H)\subset\sh_D(H)$, the \emph{remote shadow} of $V$, by removing for each $\ell\in[1,a_2]$ the (up to) $b$ horizontal edges of height $\ell-1$ immediately preceding $v_\ell$.
    \end{enumerate}
  \end{definition}
  ...
  \begin{definition}
    Consider $H\sqcup V\subset D$ with $H\subset D_1$ and $V\subset D_2$.
    \begin{enumerate}
      \item For $0 < j,d \le a_1$ with $h_j\in H$, denote by $\rsh_D(H)_{j;d}$ the set of all $v\in\rsh_D(H)$ of depth $d$ such that $v\in\sh(h_j;H)$ and $h_j$ is the first horizontal edge before $v$ with this property.
        Define the \emph{local remote shadow} of the edge $h_j$ by $\rsh_D(h_j;H):=\bigsqcup_{d\in[1,a_1]} \rsh_D(H)_{j;d}$.
      \item For $0 < k \le a_2$ with $v_k\in V$ and $0\le \ell < a_2$, denote by $\rsh_D(V)_{k;\ell}$ the set of all $h\in\rsh_D(V)$ of height $\ell$ such that $h\in\sh(v_k;V)$ and $v_k$ is the first vertical edge after $h$ with this property.
        Define the \emph{local remote shadow} of the edge $v_k$ by $\rsh_D(v_k;V):=\bigsqcup_{\ell\in[0,a_2-1]} \rsh_D(V)_{k;\ell}$.
    \end{enumerate}
  \end{definition}
