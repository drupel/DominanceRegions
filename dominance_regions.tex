...
\begin{document}
  ...
  \begin{lemma}
    For $i,j\in\ZZ$ and $\varepsilon\in\{\pm\}$, we have
    \begin{equation}
      \label{eq:long Chebyshev recursion}
      u_{i+j,\varepsilon}=\begin{cases} u_{j+1,\varepsilon}u_{i,-\varepsilon}-u_{j,-\varepsilon}u_{i-1,\varepsilon} & \text{if $j$ is odd;}\\ u_{j+1,-\varepsilon}u_{i,\varepsilon}-u_{j,\varepsilon}u_{i-1,-\varepsilon} & \text{if $j$ is even.} \end{cases}
    \end{equation}
  \end{lemma}
  \begin{proof}
    We work by induction on $j$, the cases $j=0,1$ being tautological and reproducing the defining recursion respectively.
    Using the claim for $j>0$ and then the defining recursion twice, we get
    \begin{align*}
      u_{i+j+1,\varepsilon}&=\begin{cases} u_{j+1,\varepsilon}u_{i+1,-\varepsilon}-u_{j,-\varepsilon}u_{i,\varepsilon} & \text{if $j$ is odd;}\\ u_{j+1,-\varepsilon}u_{i+1,\varepsilon}-u_{j,\varepsilon}u_{i,-\varepsilon} & \text{if $j$ is even;} \end{cases}\\
      &=\begin{cases} (u_{2,-\varepsilon}u_{j+1,\varepsilon}-u_{j,-\varepsilon})u_{i,\varepsilon}-u_{j+1,-\varepsilon}u_{i-1,\varepsilon} & \text{if $j$ is odd;}\\ (u_{2,\varepsilon}u_{j+1,-\varepsilon}-u_{j,\varepsilon})u_{i,-\varepsilon}-u_{j+1,\varepsilon}u_{i-1,-\varepsilon} & \text{if $j$ is even;} \end{cases}\\
      &=\begin{cases} u_{j+2,-\varepsilon}u_{i,\varepsilon}-u_{j+1,-\varepsilon}u_{i-1,\varepsilon} & \text{if $j+1$ is even;}\\ u_{j+2,\varepsilon}u_{i,-\varepsilon}-u_{j+1,\varepsilon}u_{i-1,-\varepsilon} & \text{if $j+1$ is odd;} \end{cases}
    \end{align*}
    which is the claimed recursion for $j+1$.
    These calculations can be reverses to show the result for $j<0$.
  \end{proof}
  ...
  \begin{proof}
    ...
    \end{align*}
    where the last equality uses \eqref{eq:long Chebyshev recursion}.

    Similarly, using $\phi_{t_1}(\lambda)=(-\lambda_1,c\lambda_1+\lambda_2)$ for $\lambda\in\cI$, together with the basic Chebyshev recursion and the equality $\phi_{2j+1}=\phi_{t_1}\phi_{t_2}^j$ gives the claimed formula for $\phi_{t_{2j+1}}$ from that of $\phi_{t_{2j}}$.
  \end{proof}
  ...
  \begin{lemma}
    For $i\ge2$ and $\lambda\in\cI$, the region $\phi_{t_i}^{-1}\cC_{t_i}(\phi_{t_i}\lambda)\subset\RR^2$ is the polytope determined by the following inequalities:
    \begin{align}
  ...
  \begin{proof}
    ...
    Both of the boundary rays corresponding to the inequalities (a) and (d) lies entirely in the region where $\phi_{t_2}^{-1}$ acts according to the matrix $\left[ \begin{array}{cc} -1 & -b\\ c & bc-1 \end{array}\right]$.
    Thus applying Lemma~\ref{le:transformed inequalities} to (a) gives the inequality 
    \[(u_{3,+}u_{2k,-}-u_{2,-}u_{2k-1,+})x+(u_{2,+}u_{2k,-}-u_{1,-}u_{2k-1,+})y\le u_{i,-}\lambda_1+u_{i-1,+}\lambda_2,\]
    which is the inequality (a) for $k+1$ by Lemma~\ref{eq:long Chebyshev recursion}, while applying this to (d) gives the inequality 
    \[(-u_{3,+}u_{2k-1,-}+u_{2,-}u_{2k-2,+})x+(-u_{2,+}u_{2k-1,-}+u_{1,-}u_{2k-2,+})y\le u_{i+1,-}\lambda_1+u_{i,+}\lambda_2,\]
    which is the inequality (d) for $k+1$ again by Lemma~\ref{eq:long Chebyshev recursion}.
    Similarly, the boundary segment corresponding to (b) lies entirely in the region where $\phi_{t_2}^{-1}$ acts according to the matrix $\left[ \begin{array}{cc} -1 & 0\\ c & -1 \end{array}\right]$.
    Thus applying Lemma~\ref{le:transformed inequalities} to (c) gives the inequality 
    \[(u_{1,+}u_{2k-1,-}-u_{2,-}u_{2k,+})x+(-u_{0,+}u_{2k-1,-}-u_{1,-}u_{2k,+})y\le u_{i+1,-}\lambda_1+u_{i,+}\lambda_2,\]
    ...
    For $i=2j+1$, $j>0$, and $\lambda\in\cI$, we get $\cC_{t_i}(\phi_{t_i}\lambda)\subset\RR^2$ is given by the inequalities 
    \[ -x\le u_{2j+1,+}\lambda_1+u_{2j,+}\lambda_2 \qquad\text{and}\qquad y\le u_{2j+2,-}\lambda_1+u_{2j+1,-}\lambda_2. \]
  \end{proof}
