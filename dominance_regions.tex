...
\begin{document}
  ...
  Given a Dyck path $D=D_1\sqcup D_2$ in the lattice rectangle $[0,a_1]\times[0,a_2]$, write $\bar{D}=\bar{D}_1 \sqcup \bar{D}_2$ for the Dyck path in the lattice rectangle $[0,a_2]\times[0,a_1]$ with $\bar{D}_1=a_1+a_2+1-D_2$ and $\bar{D}_2=a_1+a_2+1-D_1$.
  ...
  \begin{corollary}
    Consider $H\sqcup V\subset D$ with $H\subset D_1$ and $V\subset D_2$.
    \begin{enumerate}
      \item For $h\in H$ and a vertical edge $v\in D(h;H)$, the condition (HGC) is not satisfied for the pair $(h,v)$ but for any $V\in\cC_D(H)$ the condition (VGC) is satisfied for the pair $(h,v)$.
        In particular, $D(v;V)$ is a proper subpath of $hv$ for any $V\in\cC_D(H)$.
      \item For $v\in V$ and a horizontal edge $h\in D(v;V)$, the condition (VGC) is not satisfied for the pair $(h,v)$ but for any $H\in\cC_D(V)$ the condition (HGC) is satisfied for the pair $(h,v)$.
        In particular, $D(h;H)$ is a proper subpath of $hv$ for any $H\in\cC_D(V)$.
    \end{enumerate}
  \end{corollary}
  \begin{proof}
    We prove (1), the proof of (2) is similar.
    By Lemma~\ref{le:shadow statistics}, $f_H(he)>0$ for every proper subpath of $D(h;H)=hv$ and so the condition (HGC) is not satisfied for the path $hv$, but then, since $V\in\cC_D(H)$, the condition (VGC) must be satisfied.
  \end{proof}
  ...
  \begin{theorem}
    ...
    For $a_1,a_2\in\ZZ_{\ge0}$ with $(-a_1+ba_2,-a_2)\in\cI$, we have $\mu_{V,b}\big(D^{dom}[a_1,a_2]\big)=\vv{D}^{dom}[a_2,ba_2-a_1]$ and $\mu_{H,c}\big(D^{dom}[a_1,a_2]\big)=\vv{D}^{dom}[ca_1-a_2,a_1]$.
  \end{theorem}
  \begin{proof}
