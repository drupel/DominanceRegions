...
\begin{document}
  ...
  \begin{definition}
    ...
    Given a Dyck path $D=D[a_1,a_2]$ which is horizontally-adapted to $b$, define a Dyck path $\mu_V(D)$ inside the lattice rectangle $[0,a_2]\times[0,ba_2-a_1]$ by replacing each vertical edge, together with the $\ell$ horizontal edges which immediately precede it, by a horizontal edge followed by $b-\ell$ vertical edges.

    Given a Dyck path $D=D[a_1,a_2]$ which is vertically-adapted to $c$, define a Dyck path $\mu_H(D)$ inside the lattice rectangle $[0,ca_1-a_2]\times[0,a_1]$ by replacing each horizontal edge, together with the $\ell$ vertical edges which immediately follow it, by $c-\ell$ horizontal edges followed by a vertical edge.
  \end{definition}

  \begin{lemma}\mbox{}
    \begin{enumerate}
      \item If $a_1 < b a_2$, then $\mu_V\big(D^{max}[a_1,a_2]\big)=D^{max}[a_2,ba_2-a_1]$.
      \item If $a_2 < c a_1$, then $\mu_H\big(D^{max}[a_1,a_2]\big)=D^{max}[ca_1-a_2,a_1]$.
    \end{enumerate}
  \end{lemma}
  \begin{proof}
  ...
  \begin{lemma}
    ...
    For $D^{st}:=D_1\sqcup D_2$ with $D_2=[1,a_2]$ and $D_1=[1,a_1]+a_2$, the element $x_D$ coincides with the corresponding standard monomial basis element.
  \end{lemma}
  ...
  \begin{definition}
    ...
    For $a_1,a_2\in\ZZ_{\ge0}$ with $(-a_1,-a_2+ca_1)\in\cI'$, define the \emph{dual dominant Dyck path} $D^{dom^\vee}=D^{dom^\vee}[a_1,a_2]$ to be the maximal Dyck path starting from $(0,0)$ and terminating at $(a_1,a_2)$ while staying below the lines $y=\frac{bc+\sqrt{bc(bc-4)}}{2c}x$ and $y-a_2=\frac{bc-\sqrt{bc(bc-4)}}{2c}(x-a_1)$.
  \end{definition}
  \begin{remark}
  ...
  \begin{theorem}
    For $a_1,a_2\in\ZZ_{\ge0}$ with $(-a_1+ba_2,-a_2)\in\cI$, we have $\mu_H\big(D^{dom}[a_1,a_2]\big)=D^{dom^\vee}[a_2,ba_2-a_1]$ and $\mu_V\big(D^{dom}[a_1,a_2]\big)=D^{dom^\vee}[ca_1-a_2,a_1]$.
  \end{theorem}
