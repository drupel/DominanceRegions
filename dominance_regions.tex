...
\begin{figure}[h!]
  \centering
    \newcommand{\setconstants}{
      ...
      \tikzmath{ \xmin = -6; \xmax = 6; \ymin = -4; \ymax = 8;}
  ...
  \begin{tikzpicture}[scale=.5]
      ...
      \fill [fill=red!30, fill opacity=0.5]  ( \x+\b*\y , -\c*\x - \b*\c*\y + \y)  -- (\bcps*\x/\s + \bcps*\b*\y/2/\s, -\bcps*\c*\x/2/\s - \bcps*\b*\c*\y/2/\s + \bcps*\y/\s) -- ({\bcms/(2*\c)*\y+\x},0) -- (0,{\bcms/(2*\b)*\x+\y}) -- cycle;
      \fill [fill=green!30, fill opacity=0.5] (\x,\y) -- ({\x+\bcps/(2*\c)*\y},0) -- ({\bcps/(2*\s)*(2*\x+\b*\y)},{\bcps/(2*\s)*(\c*\x+2*\y)}) -- (0,{\bcps/(2*\b)*\x+\y}) -- cycle;
      \draw [red, thick, join=round]  ( \x+\b*\y , -\c*\x - \b*\c*\y + \y)  -- (\bcps*\x/\s + \bcps*\b*\y/2/\s, -\bcps*\c*\x/2/\s - \bcps*\b*\c*\y/2/\s + \bcps*\y/\s) -- ({\bcms/(2*\c)*\y+\x},0) -- (0,{\bcms/(2*\b)*\x+\y}) -- cycle;
      \draw [green, thick, join=round] (\x,\y) -- ({\x+\bcps/(2*\c)*\y},0) -- ({\bcps/(2*\s)*(2*\x+\b*\y)},{\bcps/(2*\s)*(\c*\x+2*\y)}) -- (0,{\bcps/(2*\b)*\x+\y}) -- cycle;
...
\label{sec:dominance inequalities}
  ...
  \begin{lemma}
    ...
    \end{align}
    and by the following inequalities when $k<0$: \saySS{the third equation here is wrong!}
    \begin{align*}
