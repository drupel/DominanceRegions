...
\begin{document}
  ...
  Write $D=D_1\sqcup D_2$, where $D_1$ is the set of horizontal edges and $D_2$ is the set of vertical edges.
  ...
  \end{lemma}
  \begin{proof}
    The proof is identical to the proof of \cite[Lemma 2.4]{rupel} with appropriate notation changes.
  \end{proof}

  For $e<e'\in D$, write $ee'$ for the subpath of $D$ beginning with $e$ and ending with $e'$.
  \begin{definition}
    ...
    \[e\ne v \quad \text{and} \quad |he\cap D_2|=c|he\cap D_1|\]
    \[e\ne h \quad \text{and} \quad |ev\cap D_1|=b|ev\cap D_2|\]
  ...
  \begin{theorem}[\cite{lee-li-zelevinsky}]
    For $(a_1,a_2)=(u_{m+1,+},u_{m,-})$ or $(u_{m,+},u_{m+1,-})$ for $m\ge0$, the element $x_D[a_1,a_2]$ is a non-initial cluster variable.
    Otherwise, the element $x_{D_\WA}$ is an element of the greedy basis
  \end{theorem}
  ...
  \begin{remark}
    ...
    The lines in the definition above meet at the point $\left( \frac{2ba_2-\big(bc-\sqrt{bc(bc-4)}\big)a_1}{2\sqrt{bc(bc-4)}} , \frac{\big(bc+\sqrt{bc(bc-4)}\big)a_2-2ca_1}{2\sqrt{bc(bc-4)}} \right)$.
  \end{remark}
  ...
  \begin{definition}
    For $s>0$, define the \emph{Dyck path prefix} $\overrightarrow{D}^s$ as the maximal Dyck path with $s$ vertical edges starting from $(0,0)$ and terminating at $(d,s)$ while staying below the line $y=\frac{bc+\sqrt{bc(bc-4)}}{2b}x$, where $d=\left\lceil s \frac{bc-\sqrt{bc(bc-4)}}{2c}\right\rceil$.
    Call this $d$ the \emph{depth} of $s$.

    For $t>0$, define the \emph{Dyck path suffix} $\overleftarrow{D}^t$ as the maximal Dyck path with $t$ horizontal edges starting from $(0,0)$ and terminating at $(t,h)$ while staying below the line $y-h=\frac{bc-\sqrt{bc(bc-4)}}{2b}(x-t)$, where $h=\left\lceil t \frac{bc-\sqrt{bc(bc-4)}}{2b}\right\rceil$.
  ...
  \begin{lemma}\mbox{}
    \begin{itemize}
      \item For $s>0$ and any $m \ge 1$ so that $s < u_{m+1,-}$, the Dyck path prefix $\overrightarrow{D}^s$ is an initial segment of the maximal Dyck path $D^{max}[u_{m,+},u_{m+1,-}]$.
      \item For $t>0$ and any $m \ge 1$ so that $t < u_{m+1,+}$, the Dyck path suffix $\overleftarrow{D}^t$ is a final segment of the maximal Dyck path $D^{max}[u_{m+1,+},u_{m,-}]$.
    \end{itemize}
  \end{lemma}
  ...
  \begin{thebibliography}{XXX}
    \bibitem{rupel}
      D.~Rupel: Noncommutative Laurent Phenomenon
  \end{thebibliography}
