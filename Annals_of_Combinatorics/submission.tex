...
\title[Dominance Regions for Rank Two Cluster Algebras]{Dominance Regions for Rank Two Cluster Algebras}
...
\author*[1]{\fnm{Dylan} \sur{Rupel}}\email{rupel.dylan@pusd.us}

\author*[2]{\fnm{Salvatore} \sur{Stella}}\email{salvatore.stella@univaq.it}


\affil[1]{\orgdiv{Math Academy}, \orgname{Pasadena Unified School District}, \orgaddress{\street{351 S. Hudson Ave.}, \city{Pasadena}, \postcode{91109}, \state{California}, \country{United States}}}

\affil[2]{\orgdiv{Dipartimento di Ingegneria e Scienze dell'Informazione e Matematica}, \orgname{Università degli Studi dell'Aquila}, \orgaddress{\street{Via Vetoio}, \city{L'Aquila}, \postcode{67100}, \state{AQ}, \country{Italy}}}
...
\abstract{We study the polygons defining the dominance order on $\bfg$-vectors in cluster algebras of rank 2.\hfill~}
...
\keywords{cluster algebra, dominance order, pointed bases}

\pacs[MSC Classification]{13F60}
...
\begin{figure}[h!]
  ...
  \begin{caption}
    {
    The dominance region $\cP_\lambda$, the opposite dominance region $\cP'_\lambda$, and the maximal support region $\cS_\lambda$ for $\lambda=(4,-3)$ when $b=3$ and $c=2$.
    Highlighted lattice points in the region bounded by solid black lines and dashed lines represent the maximal support of an element pointed at $\lambda$; the dotted lines indicate which lattice points are in the support of the corresponding greedy basis element.
    }
  \end{caption}
\end{figure}
\vspace{-1em}
...
\section{Introduction}\label{sec1}
  ...
  By doing so we shed more light on the conbinatorial structure of such bases.
  ...
  \end{equation}
  with $\rho_{0,0}=1$ and $\rho_{\alpha_0,\alpha_1}\in\ZZ$.
  This definition agrees  with the notion of $\bfg$-vector from \cite{FZ07} defined for cluster variables.
  ...
  We give an explicit description of the dominance relation among $\bfg$-vectors.
  ...
  \begin{theorem}
    ...
    If $\lambda$ lies outside of $\cI$, then the dominance region $\cP_\lambda$ is the point $\lambda$.
    Otherwise the dominance region $\cP_\lambda$ of the $\bfg$-vector $\lambda=(\lambda_0,\lambda_1)$ is the polygon consisting of those $\mu=(\mu_0,\mu_1)\in\RR^2$ satisfying the following inequalities:
    {
      ...
      \[
        \begin{array}{rcccl}
          ...
          \\
          0 & \!\leq\! & -\frac{b c-\sqrt{b c (b c-4)}}{2 b}(\mu_0-\lambda_0)+(\mu_1-\lambda_1) & \!\leq\! & c\lambda_0;
          \\
          0 & \!\leq\! &  -(\mu_0-\lambda_0)-\frac{b c-\sqrt{b c (b c-4)}}{2 c}(\mu_1-\lambda_1) & \!\leq\! & \frac{b c+\sqrt{b c (b c-4)}}{2c}c\lambda_0+b\lambda_1;
          \\
          0 & \!\leq\! & -(\mu_0-\lambda_0) + \frac{b c-\sqrt{b c (b c-4)}}{2 c} (\mu_1-\lambda_1) & \!\leq\! & -b \lambda_1.
        \end{array}
      \]
    }
  \vspace{-1.05em}
  \end{theorem}
  \begin{remark}
    Empirical calculations reveal that for certain pairs of notable bases (e.g. greedy and triangular) most of the coefficients $q_{\lambda,\mu}$ are zero.
  ...
  \begin{remark}
    ...
    Unfortunately, at the moment, we are unable to explain the reason behind it.
  \end{remark}
  ...
  \begin{theorem}
    ...
    \begin{enumerate}
      \item If $\lambda$ lies outside of $\cI$, then the support of $x_\lambda$ is precisely the points of the form $\lambda+(b \alpha_0 ,c \alpha_1)$, $\alpha_0,\alpha_1\in\ZZ$, inside the region $\cS_\lambda$ given as follows:
        \begin{enumerate}
          \item If $0\le\lambda_0,\lambda_1$, then $\cS_\lambda$ is just the point $\lambda=\lambda'$.
          \item If $\lambda_1 < 0$ and $0\le\lambda_0+b\lambda_1$, then $\cS_\lambda$ is the segment joining $\lambda$ and $\lambda'=(\lambda_0+b\lambda_1,\lambda_1)$.
          \item If $\lambda_0 < 0$ and $0\le\lambda_1$, then $\cS_\lambda$ is the segment joining $\lambda$ and $\lambda'=(\lambda_0,-c\lambda_0+\lambda_1)$.
          \item If $\lambda_0,\lambda_1 < 0$, then $\cS_\lambda$ has  vertices $\lambda$, $(\lambda_0+b\lambda_1,\lambda_1)$, $\lambda'=\big(\lambda_0+b\lambda_1,-c\lambda_0-(bc-1)\lambda_1\big)$, and ${(\lambda_0,-c\lambda_0+\lambda_1)}$.
          \item If $\lambda_0 > 0$, $\lambda_0+b\lambda_1 < 0$, and $-c\lambda_0-(bc-1)\lambda_1\le 0$, then $\cS_\lambda$ has  vertices $\lambda$, $(\lambda_0+b\lambda_1,\lambda_1)$, $\lambda'=\big(\lambda_0+b\lambda_1,-c\lambda_0-(bc-1)\lambda_1\big)$, and $\big((bc+1)\lambda_0-b^2c\lambda_1,-c\lambda_0-(bc-1)\lambda_1\big)$.
          \item If $\lambda_0 > 0$, $\lambda_0+b\lambda_1 < 0$, and $0 < -c\lambda_0-(bc-1)\lambda_1$, then $\cS_\lambda$ has  vertices $\lambda$, $(\lambda_0+b\lambda_1,\lambda_1)$, $\lambda'=\big(\lambda_0+b\lambda_1,-c\lambda_0-(bc-1)\lambda_1\big)$, and $(0,0)$.  Here the point $(0,0)$ and its adjacent open segments are excluded from $\cS_\lambda$ while both $\lambda$ and $\lambda'$ are included.
        \end{enumerate}
      \item If $\lambda$ lies inside of $\cI$, then the support of $x_\lambda$ is contained in $\cS_\lambda$ with  vertices $\lambda$, $(\lambda_0+b\lambda_1,\lambda_1)$, $\lambda'=\big(\lambda_0+b\lambda_1,-c\lambda_0-(bc-1)\lambda_1\big)$, and $\frac{bc+\sqrt{bc(bc-4)}}{2\sqrt{bc(bc-4)}}\big(2\lambda_0+b\lambda_1,-c\lambda_0-(bc-2)\lambda_1\big)$. This last point and its adjacent open segments are excluded from $\cS_\lambda$ while both $\lambda$ and $\lambda'$ are included.
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[scale=.22]
    \begin{scope}[shift={(0,0)}]
      ...
      \draw [thick, fill=black] (\x , \y)  circle (5pt);
      \draw (\xmin+1.5,\ymin+1) node{\small(1.a)};
    \end{scope}
    \begin{scope}[shift={(14,0)}]
      ...
      \draw (\xmin+1.5,\ymin+1) node{\small(1.b)};
    \end{scope}
    \begin{scope}[shift={(28,0)}]
      ...
      \draw (\xmin+1.5,\ymin+1) node{\small(1.c)};
    \end{scope}
    \begin{scope}[shift={(42, 0)}]
      ...
      \draw (\xmin+1.5,\ymin+1) node{\small(1.d)};
    \end{scope}
    \begin{scope}[shift={(0,-14)}]
      ...
      \draw (\xmin+1.5,\ymin+1) node{\small(1.e)};
    \end{scope}
    \begin{scope}[shift={(14,-14)}]
      ...
      \draw [thick, fill=white] ( \x+\b*\y , -\c*\x - \b*\c*\y + \y) circle (5pt);
      \draw (\xmin+1.5,\ymin+1) node{\small(1.f)};
    \end{scope}
    ...
    \begin{scope}[shift={(42,-14)}]
      ...
      \draw [thick, join=round, line cap=round, dash pattern=on {\gap*\pgflinewidth} off {2*\gap*\pgflinewidth} ] ( \x+\b*\y , -\c*\x - \b*\c*\y + \y) --  (\bcps*\x/\s + \bcps*\b*\y/2/\s, -\bcps*\c*\x/2/\s - \bcps*\b*\c*\y/2/\s + \bcps*\y/\s) --  (\x , \y);
      \draw [thick, fill=white]  ( \x+\b*\y , -\c*\x - \b*\c*\y + \y) circle (5pt);
      \draw (\xmin+1.5,\ymin+1) node{\small(2)};
    \end{scope}
  \end{tikzpicture}
  \begin{caption}
    {
    The maximal supports $\cS_\lambda$ as described in Theorem \ref{th:maximum support} with $\lambda$ marked by a black dot and $\lambda'$ by a white dot.
    Dashed edges indicate the maximal support of an element pointed at $\lambda$ while dotted edges indicate the support of the corresponding greedy basis element.
    The imaginary cone $\cI$ is dashed; the shaded cones correspond to the cases in Theorem \ref{th:maximum support}.
    }
    \label{fig:2}
  \end{caption}
\end{figure}
...
\label{sec:tropical}
  ...
  \[
    ...
    \end{cases}
  \]
  For $k\in\ZZ$ with $\lvert k\rvert>1$, define piecewise-linear maps
  \[\phi_k
    :=
    \begin{cases}
...
\label{sec:dominance inequalities}
  ...
  The following Lemma proves Theorem~\ref{th:dominance inequalities} for~$\lambda\not\in\cI$.
  \begin{lemma}
...
\begin{figure}[h!]
  ...
  \begin{caption}
    {
      Two examples of the regions in Lemma \ref{le:dominance inequalities}. 
      Each inequlity is represented by a shaded halfspace, darker regions cosist of points satisfying multiple inequalities.
    }
  \end{caption}
\end{figure}
\vspace{-2em}
...
\label{sec:maximum support} 
  ...
  \end{lemma}
  \vspace{-1em}
  In particular, we get an opposite dominance region $\cP'_\lambda$ for each $\bfg$-vector $\lambda$.
  \vspace{-1em}
  \begin{lemma}
  ...
  \[\left(\lambda_0 -\frac{bc+\sqrt{bc(bc-4)}}{2c} c\lambda_0-\frac{bc+\sqrt{bc(bc-4)}}{2c} \lambda_1 , 0 \right)\]
  ...
  The second point can be rewritten as
  \[\left(0, -\frac{bc+\sqrt{bc(bc-4)}}{2b} \lambda'_0-\frac{bc+\sqrt{bc(bc-4)}}{2b} b\lambda'_1+\lambda'_1 \right)\]
...
This is indicated by the solid and dotted lines in Figures \ref{fig:1} and \ref{fig:2}, where dotted lines indicate points that are excluded from the support.
  ...
  For our purposes, it is enough to observe the following closure property for the greedy support region which is evident from Figure \ref{fig:2}.
  \begin{definition}
